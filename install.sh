...
Required:
  ...
  --session-name NAME     Herdr named session
...
[[ -n $HERDR_SESSION ]] || die '--session-name is required'
...
render_config() {
  ...
  printf '\nHERDR_SESSION='
  quote_config_value "$HERDR_SESSION"
  printf '\n'
  printf 'IGNORE_PATHS=('
  for ignore_path in "${IGNORE_PATHS[@]-}"; do
...
printf '  launcher: %s\n' "$launcher_target"
printf '  config:   %s\n' "$CONFIG_FILE"
printf 'Run hremote --dry-run, then hremote.\n'
